...
const getBirthdays = {
	...
	async execute(_body: APIInteraction) {
		const u = (await db.select().from(users))
			.map((u) => {
				const nextAge =
					Math.abs(DateTime.fromISO(u.birthday).diffNow("years").years) + 1;
				return {
					...u,
			...
			.sort((a, b) => {
				const aDate = DateTime.fromISO(a.birthday).set({ year: 0 });
				const bDate = DateTime.fromISO(b.birthday).set({ year: 0 });
...
${u
	.map(
		(user) =>
			`- On the **${DateTime.fromISO(user.birthday)
				.setLocale("da-DK")
